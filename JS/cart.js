...
let cartData = {};
let dataToSend = {};
let sendBtn = document.getElementById('valider');

sendBtn.addEventListener('click', () => {
    formData.push(nom.value);
    formData.push(prenom.value);
    formData.push(address.value);
    formData.push(city.value);
    formData.push(mail.value);
...
const sendData = () => {
    sendHttpRequest('POST', 'http://localhost:3000/api/teddies/order', dataToSend)
      .then(responseData => {
